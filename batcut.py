...
def batcut(directory, filename, drow):
    ...
    with open(directory + "individual_results/_" + filename.split(".wav")[0] + "-sceneRect.csv", 'r') as calls:
        ...
        for row in csv_call:
            ...
            while start_call >= timer[ind_e]:
                ind_b += 1
                ind_e += 1
                
            if not(timer[ind_b] <= start_call and end_call <=  timer[ind_e]):
...
batcut("/home/maxime/Documents/memoire/batnpz/", "test.wav", {"Directory" : "flip", "File" : "test.wav", "Id" : "etetet"})
